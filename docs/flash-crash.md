...
12. **逐字符增量刷新时钟**：若按「新字符」尺寸清格，旧字符更宽/更高时右侧/底部会残留线条
    （advance 固定 24px，但像素范围随字符不同：`0` 画到 x+4..x+26 超出格 2px，高度 h=32..34 也各异）。
    解法：用**固定满格清除框**（x+3..x+26，高取 36），可覆盖任意前驱字符，且利用数字间 ≥2px
    像素间隙不误伤左右邻居。
13. **CJK 字体缺字**：`周`(U+5468) 未收录，`周五` 只显示 `五`。星期改用英文 `FRI`；
    农历「月」(U+6708) 已收录，`text()` 需显式拼「月」，否则显示「八初一」。
    排查缺字：把码点列表打印出来 `ord(字) in cps` 一眼确认。
