...
static Arduino_HWSPI g_lcdBus = Arduino_HWSPI(LCD_DC_GPIO, LCD_CS_GPIO, &SPI, true);
 ...
 * @return void
 */
static void lcdFlipHorizontalMirror(uint8_t rotation) {
    uint8_t madctl;
    switch (rotation) {
        case 1:
...
static void lcdEnsureInit() {
    ...
    lcdFlipHorizontalMirror(rotation);
...
auto DisplayManager::update() -> void {
    ...
    // 无 GIF 时, 每秒刷新一次时钟/天气/服务屏
    static unsigned long lastRender = 0;
    if (millis() - lastRender >= 1000UL) {
        lastRender = millis();
        ClockScreen::render(&g_lcd);
    }
}
