...
void registerApiEndpoints(Webserver* webserver) {
    ...
    // @openapi {get} /token/bootstrap version=v1 group=Authentication summary="Get current token for UI bootstrap" requiresAuth=false
    // responses=200:application/json
    webserver->raw().on("/api/v1/token/bootstrap", HTTP_GET, [webserver]() { handleTokenBootstrap(webserver); });
...
/**
 * @brief Return the current token so the web UI can bootstrap without manual entry
 *
 * Intentionally unauthenticated: it is the local-network entry point that lets the
 * browser obtain a token on first visit and store it for subsequent calls.
 */
void handleTokenBootstrap(Webserver* webserver) {
